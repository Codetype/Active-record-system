...
public class Course {
    ...
    public boolean enrollStudent(final Student student) {
        String enrollStudentSql = String.format("INSERT INTO STUDENT_COURSE (student_id, course_id) VALUES (%d, %d);", student.id(), id());
    ...
    /*public List<Student> studentList() {
        ...
        String findStudentListSql = String.format("SELECT student_id, first_name, last_name, index_number FROM STUDENT s INNER JOIN STUDENT_COURSE sc ON sc.student_id = s.student_id WHERE COURSE_ID = '%s'", id());
        try {
            ResultSet rs = QueryExecutor.read(findStudentListSql);
            while(rs.next()){
                resultList.add(new Student(rs.getInt("student_id"), rs.getString("first_name") , rs.getString("last_name"), rs.getInt("index_number")));
            }
        } catch (SQLException e) {
            e.printStackTrace();
        }
    ...
    public List<Student> studentList() {
        ...
        String findStudentListSql = String.format("SELECT STUDENT_ID FROM STUDENT_COURSE WHERE COURSE_ID = '%s'", id());
        try {
            ResultSet rs = QueryExecutor.read(findStudentListSql);
            while(rs.next()){
